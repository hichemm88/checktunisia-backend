...
CMD ["/bin/bash", "-c", "php artisan key:generate --force && php artisan migrate --force && php artisan db:seed --class=CountrySeeder --force && php artisan db:seed --class=DocumentTypeSeeder --force && php artisan db:seed --class=RolesAndPermissionsSeeder --force && php artisan db:seed --class=SubscriptionPlanSeeder --force && php artisan db:seed --class=PlatformAdminSeeder --force && php artisan db:seed --class=DemoDataSeeder --force && php artisan config:cache && php artisan route:cache && php -d variables_order=EGPCS artisan serve --host=0.0.0.0 --port=${PORT:-8000}"]
